...
Each scenario is run with a fresh RIFT state:
  - ConversationMemory is reset
  - Anchors are cleared
  - Basins (including the persisted JSON file) are cleared

This makes the comparison against the baseline easier to interpret.
"""
...
from rift.core.anchors import ANCHORS
from rift.core.engine import BASINS
...
# ============================================================
#  RIFT global reset helper
# ============================================================

def reset_rift_global_state() -> None:
    """
    Clear all RIFT global truth state:

    - In-memory anchors (ANCHORS)
    - In-memory basins (BASINS.basins)
    - Persisted basin JSON file, if present

    This ensures each scenario starts with a fresh field.
    """
    # Clear anchors
    ANCHORS.clear()

    # Clear in-memory basins
    BASINS.basins.clear()

    # Delete persisted basin file, if it exists
    basin_path = getattr(BASINS, "path", None)
    if isinstance(basin_path, str):
        try:
            if os.path.exists(basin_path):
                os.remove(basin_path)
        except Exception:
            # Benchmark should not crash on filesystem issues
            pass
...
class RIFTEngineWrapper:
    """
    OpenAI-backed implementation of RIFTEngine for benchmarking.
    ...
    def reset(self) -> None:
        # Fresh conversation memory; anchors/basins are reset at the
        # scenario level by reset_rift_global_state().
...
class BaselineEngine:
    """
    Plain LLM with conversational memory (per scenario),
    but without RIFT logic.
    """
    ...
    def answer(self, user_text: str) -> str:
        ...
        reply = self._client.chat_completion(self._messages).strip()
...
def run_scenario_for_engine(engine: QAEngine, scenario: dict) -> ScenarioResult:
    ...
    # Fresh RIFT state for this scenario (for all engines).
    # For the baseline engine, this is effectively a no-op.
    reset_rift_global_state()

    # Reset engine-local state (e.g., ConversationMemory for RIFT).
    engine.reset()
